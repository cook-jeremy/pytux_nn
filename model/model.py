...
def save_model(model):
    ...
    if isinstance(model, PuckDetector):
        return save(model.state_dict(), path.join(path.dirname(path.abspath(__file__)), 'puck.th'))
...
def load_model():
    ...
    from os import path
    r = PuckDetector()
    r.load_state_dict(load(path.join(path.dirname(path.abspath(__file__)), 'puck.th'), map_location='cpu'))
    return r
