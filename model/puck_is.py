...
def save_model(model):
    ...
    from os import path
    if isinstance(model, PuckIs):
        return save(model.state_dict(), path.join(path.dirname(path.abspath(__file__)), 'puck_is.th'))
...
def load_model():
    ...
    from os import path
    r = PuckIs()
    r.load_state_dict(load(path.join(path.dirname(path.abspath(__file__)), 'puck_is.th'), map_location='cpu'))
    return r